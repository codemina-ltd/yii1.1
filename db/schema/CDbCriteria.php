...
class CDbCriteria extends CComponent
{
     ...
     * @return static the criteria object itself
     */
    public function addColumnCondition($columns, $columnOperator = 'AND', $operator = 'AND', bool $like = false, bool $escape = false)
    {
        $params = array();
        foreach ($columns as $name => $value) {
            if ($value === null)
                ...
                if ($like) {
                    ...
                    $this->params[self::PARAM_PREFIX . self::$paramCount++] = $escape ? '%' . $value . '%' : $value;
                } else {

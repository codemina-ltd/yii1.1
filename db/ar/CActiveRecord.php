 ...
 * @property false|string update_date
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
...
abstract class CActiveRecord extends CModel
{
     ...
     * @return CActiveRecord
     */
    public function paginate(CDbCriteria $criteria, PageDTO $page, int $total, array $with = []): self
    {
        $this->setPaginator(
            (new CPagination($total))
                ->setCurrentPage($page->getNumber() ?? 0)
                ->setPageSize($page->getSize() ?? 10)
                ->setFilteredCount($this->with($with)->count($criteria))
                ->applyLimit($criteria)
        );
